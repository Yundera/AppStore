...
Lidarr is protected by **its own built-in Forms authentication**, seeded on so that it
...
A second deviation: **volumes outside `/DATA/AppData/lidarr/`** — the container mounts
`${DATA_ROOT:-/DATA}/Media/Music` and `${DATA_ROOT:-/DATA}/Downloads` read-write.

## Why it is necessary

Lidarr is an API-first application, and the browser UI and the machine API share one
...
- **Exempt `/api/*`** — the exemption covers the entire administrative surface. Every
  destructive operation Lidarr can perform is an `/api/v1/…` call, so a gate that lets
...
**Media and download mounts.** Lidarr's whole job is to move finished downloads out of `/DATA/Downloads` and into the
music library at `/DATA/Media/Music`, renamed and sorted. Both paths must be writable, and
they must be the *shared* store folders — the point is that Jellyfin, Navidrome, qBittorrent
and Lidarr all see one library rather than several private copies.
...
- **Auth is enabled before first reach, not after.** `seed/config/config.xml.tmpl` ships
  ...
  into `/DATA/AppData/lidarr/config/config.xml` at install — before the container ever
...
- **A unique password per app, per install.** `x-compose-app.secrets` generates
  `LIDARR_ADMIN_PASSWORD` as `alnum:24` (~143 bits) once into the app's `.env`. It is not
...
- **`AuthenticationMethod=Forms` with no seeded credential** — rejected because it is a hard
  lockout. With no user row in `lidarr.db` the login page offers no registration path and
...
- `seed/` is **create-if-absent**, so `config.xml` is written once and never rewritten.
  Lidarr consumes `<Username>`/`<Password>` on first boot, creates the user in `lidarr.db`
...
- All app state lives under `/DATA/AppData/lidarr/config`, which is covered by the app's
...
- `/DATA/Media/` and `/DATA/Downloads` are the user's own directories, shared with the other
  media apps and untouched by installing or removing Lidarr.
