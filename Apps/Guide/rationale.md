...
2. **Runs as `user: 0:0` (root).** The single service is `ghcr.io/yundera/guideapp:1.0.3` — an nginx image built `FROM nginx:1.27-alpine` with the static site and catalogue baked in — bound to port 80, which requires root to bind the privileged port.
...
Concretely, the container is a plain nginx serving `index.html`, `guide.html`, a bundle of Markdown docs under `/docs`, a public catalogue JSON, and a small set of screenshots. There is no database, no user state on the server, and nothing behind an auth gate that would benefit from protecting. Its whole purpose is to be the first URL a new PCS owner opens after setup, to help them decide what to install — putting Authelia SSO in front of it would only add friction to the *"just landed on my PCS, what now?"* moment it is designed for.
...
The container is built `FROM nginx:1.27-alpine` (the only additions are the static site and the baked-in catalogue; the nginx binary and its default entrypoint are untouched) and listens on port 80, which is privileged and requires root to bind. Guide has **no `/DATA` volumes** — no `/DATA/AppData/guide/`, no user-directory mounts — so the "root containers are acceptable when volumes map exclusively to AppData" allowance in `CONTRIBUTING.md` § Permission Strategy applies *a fortiori* (nothing on the host to touch at all). We did not switch to `nginxinc/nginx-unprivileged` because bind-mount-free static serving on port 80 is the exact case where root inside a rootless-network-namespaced container is safe and idiomatic; rebasing our Dockerfile on that image would also force a port rewrite in the Caddy labels for no measurable security gain.
...
- **No server-side outbound network at runtime.** The catalogue is baked into the image at build time (`Dockerfile` stage 1 clones `Yundera/AppStore` once, syncs, and copies `catalog.json`). Runtime nginx never phones home.
- **One browser-side third-party request.** The shipped page links the Comfortaa webfont from Google Fonts, so each visitor's *browser* fetches `https://fonts.gstatic.com/s/comfortaa/...woff2` on load and discloses its IP to Google; nothing else is sent and the page falls back to a system font when the request fails, so it stays usable on an air-gapped PCS. The server itself still makes no outbound call. Self-hosting the font inside the `guideapp` image is the intended fix and is tracked upstream — it cannot be done from this store repository, which ships no page assets.
- **Small resource footprint.** 64 MB memory cap, `cpu_shares: 80`. Trivial blast radius even if compromised.
- **No `/DATA` volumes at all.** Nothing to leak.
- **Specific image tag.** Pinned to the explicit release tag `ghcr.io/yundera/guideapp:1.0.3` — no `:latest` in the shipped compose.
...
- **`user: $PUID:$PGID` on the nginx base image** — fails at startup: master nginx can no longer bind port 80 (`permission denied`).
